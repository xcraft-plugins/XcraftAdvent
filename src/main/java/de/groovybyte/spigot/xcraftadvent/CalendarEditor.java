...
public class CalendarEditor {
    ...
    public CalendarEditor(XcraftAdvent plugin, CalendarManager man) {
        this.plugin = plugin;
        this.man = man;
        this.inv = Bukkit.createInventory(null, InventoryType.CHEST, "Türchenfüllen");
    }
    ...
    protected void setSelectionInv() {
        ...
        for (int i = 1; i <= 24; ++i) {
            ...
            if (this.man.config.contains("rewards.door" + i + ".locations")) {
                List<Location> locations = (List<Location>) this.man.config.getList("rewards.door" + i + ".locations");
                if (locations != null) {
            ...
            if (door.isEmpty()) {
                ...
                if (door.hasItems()) {
                    ...
                    var x = Arrays.stream(door.getItems())
                        .limit(5L)
                        .map((is) -> is.getType().toString());
                    if((items.length > 5))
                        var10001 = x.collect(Collectors.joining(", ", "", "..."));
                    else
                        var10001 = x.collect(Collectors.joining(", "));
            ...
            meta.setDisplayName(String.format("Türchen %s", i));
            meta.setLore(l);
    ...
    public void addDoorInventoryHolder(int day, Location inventoryLocation) {
        List<Location> tmp = (List<Location>) this.man.config.getList("rewards.door" + day + ".locations");
        if (tmp == null) {
            tmp = new ArrayList(1);
        }
